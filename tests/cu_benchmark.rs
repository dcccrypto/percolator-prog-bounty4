// Kani cannot resolve `solana_program_runtime` (host-only crate, not in Kani's
// dependency graph). Gate the entire file so `cargo kani --tests` skips it.
#![cfg(not(kani))]
#![allow(
    dead_code,
    unused_imports,
...
impl TestEnv {
    ...
    fn try_crank_inner(&mut self, cu_limit: Option<u32>) -> Result<(u64, Vec<String>), String> {
        let caller = Keypair::new();
        self.svm.airdrop(&caller.pubkey(), 1_000_000_000).unwrap();
...
fn benchmark_worst_case_scenarios() {
    ...
    println!("  Insurance funded → engine takes MTM path (not force_realize). Varies #liq users");
    println!("  within the first 128-candidate window; rest are safe.");
    {
        ...
        println!("  may bite earlier. Keeper should cap candidates/crank to whichever is smaller.");
    }
